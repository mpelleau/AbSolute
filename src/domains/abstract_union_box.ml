...
module Union_Box(UI:UITV) = (struct
  ...
  (* returns a box included in its argument, by removing points such that
     ...
     can raise Bot_found *)
  let rec refine (a:t list) (e:bexpr) (x:i) : t list =
    match e with
    | BVar v -> 
        List.fold_left (fun l env -> List.append l 
          [(try Env.add v (debot (I.meet x (fst (find v env)))) env
          with Not_found -> failwith ("variable not found: "^v))]
        ) [] a;
...
module UBoxF = Union_Box(Union_itv.U_ItvF)
