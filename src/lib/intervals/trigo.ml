...
(* This modules defines sound operators for trogonometrical functions It uses
   floating point precision It plugs itself over a interval arithmetic module.
...
module Make (I : Itv_sig.ITV) = struct
  ...
  let acos_up r = if -1. <= r && r <= 1. then Some (acos r) else None

  let acos_down r = if -1. <= r && r <= 1. then Some (-.acos (-.r)) else None

  (************************)
  (* ARCSIN APPROXIMATION *)
  (************************)

  let asin_up r = if -1. <= r && r <= 1. then Some (asin r) else None

  let asin_down r = if -1. <= r && r <= 1. then Some (-.asin (-.r)) else None
  ...
  let print_fitv fmt (l, u) =
    Format.fprintf fmt "[%a; %a]" Format.pp_print_float l Format.pp_print_float
      u
  ...
  (* - Change means than the function f is not monotonic on the given interval.
     the boolean indicates if the function is firstly increasing (true) or
     decreasing(false) *)
  type monotony = Incr | Decr | Change of bool
  ...
  (* given an interval i, and a function f and its monotony, return the image of
     i by f *)
  ...
  let cosmonotony (a, b) =
    ...
    let mono_once (a, b) =
      ...
      if m_sin_a = 0. then if m_sin_b < 0. then Decr else Incr
      else if m_sin_a < 0. then if m_sin_b <= 0. then Decr else Change false
      else if m_sin_b >= 0. then Incr
      else Change true
    in
    let rec mono (a, b) =
      ...
      else (* monotony changes at most once *)
        mono_once (a, b)
    in
    mono (a, b)
  ...
  let acos_itv i =
    ...
    if 1. < l || u < -1. then None
      ...
      and u' = if l < -1. then pi_up else Option.get (acos_up l) in
      Some (I.of_floats l' u')

  (* sinus of an interval *)
  let sin_itv i = cos_itv (I.sub i pihalf_itv)
  ...
  (* bring an interval to target (the lower bound ) interval size should be
     smaller than maxsize, it raises Exit if not *)
  let normalize target maxsize i =
    if maxsize <= I.float_size i then raise Exit
      ...
      if maxsize <= I.float_size i' then raise Exit else (i', dist)
  ...
  let filter_cos = filter (I.of_floats 0. twopiup) arcos_0_2pi
