...
(* real itv conversion to integer itv. the biggest inner element is returned.
   may be bottom *)
let to_int (r : R.t) : I.t option =
...
let force_real (i : t) : R.t = match i with Real r -> r | Int i -> to_float i

let dispatch f_int f_real = function Int x -> f_int x | Real x -> f_real x
...
let of_ints (x1 : int) (x2 : int) : t = Int (I.of_ints x1 x2)

let of_floats (x1 : float) (x2 : float) : t = Real (R.of_floats x1 x2)

let of_int (x1 : int) : t = of_ints x1 x1

let of_float (x1 : float) : t = of_floats x1 x1

let of_rats (m1 : Mpqf.t) (m2 : Mpqf.t) : t = Real (R.of_rats m1 m2)

let of_rat (m1 : Mpqf.t) : t = of_rats m1 m1

(* maps empty intervals to explicit bottom *)
let check_bot (x : t) : t option =
  match x with
  | Int x -> Option.map int (I.check_bot x)
  | Real x -> Option.map real (R.check_bot x)
...
let float_size (i : t) : float = dispatch I.float_size R.float_size i
...
let is_positive (itv : t) : bool =
  match itv with Int (a, _) -> a >= 0 | Real r -> R.subseteq r R.positive

(* retruns true if the interval is negative (large sense), false otherwise *)
let is_negative (itv : t) : bool =
  match itv with Int (_, b) -> b <= 0 | Real r -> R.subseteq r R.negative

let contains_float (x : t) (f : float) : bool =
  match x with Int x -> I.contains_float x f | Real x -> R.contains x f
...
let intersect (x1 : t) (x2 : t) : bool =
  ...
  | Int x1, Real x2 | Real x2, Int x1 -> (
    match to_int x2 with None -> false | Some x2 -> I.intersect x1 x2 )

let is_singleton (itv : t) : bool = dispatch I.is_singleton R.is_singleton itv

(* mesure *)
(* ------ *)
let range (x : t) : float =
  match x with Int x -> float (I.range x) | Real x -> R.range x
...
let div (x1 : t) (x2 : t) : t option =
  ...
  | Int x2 ->
      ...
      else Option.map real (R.div x1 (to_float x2))
  | Real x2 -> Option.map real (R.div x1 x2)

(* returns valid value when the exponant is a singleton positive integer. fails
   otherwise *)
let pow (x1 : t) (x2 : t) : t =
  match (x1, x2) with
...
(* function calls (sqrt, exp, ln ...) are handled here : given a function name
   and and a list of argument, it returns a possibly bottom result *)
let eval_fun (name : string) (args : t list) : t option =
...
let filter i_f r_f (x1 : t) (x2 : t) : (t * t) Consistency.t =
  ...
  | Int x1, Real x2 -> (
      ...
      try
        Consistency.map
          (fun (x1, x2) -> (Int (Option.get (to_int x1)), Real x2))
          (r_f x1 x2)
      with _ -> Unsat )
  | Real x1, Int x2 -> (
      ...
      try
        Consistency.map
          (fun (x1, x2) -> (Real x1, Int (Option.get (to_int x2))))
          (r_f x1 x2)
      with _ -> Unsat )
...
(* given the interval argument(s) and the expected interval result of a numeric
...
(* filtering function calls like (sqrt, exp, ln ...) is done here : given a
   function name, a list of argument, and a result, it remove points that cannot
   satisfy the relation : f(arg1,..,argn) = r; it returns a possibly bottom
   result *)
let filter_fun (name : string) (args : t list) (res : t) : t list option =
  let args = List.map (function Real x -> x | Int x -> to_float x) args in
  let float_res = match res with Real x -> x | Int x -> to_float x in
  Option.map (List.map real) (R.filter_fun name args float_res)

(* generate a random float within the given interval *)
let spawn (x : t) : float =
  match x with Int x -> float (I.spawn x) | Real x -> R.spawn x
