...
module Make (I : Itv_sig.ITV_EVAL) = struct
  ...
  let filter_sqrt (i : t) (r : t) : t option =
    let rr = mul r r in
    if is_positive i then meet_opt i rr else meet_opt i (join i rr)

  (* r = exp i => i = ln r *)
  let filter_exp (i : t) (r : t) : t option = Option.bind (ln r) (meet_opt i)

  (* r = ln i => i = exp r *)
  let filter_ln (i : t) (r : t) : t option = meet_opt i (exp r)
  ...
  (* r = nroot i => i = r ** n *)
  let filter_root i r n = Tools.(merge_bot (meet_opt i (pow r n)) (Some n))
